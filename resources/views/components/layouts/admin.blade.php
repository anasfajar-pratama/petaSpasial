        ...
        <div x-data="{ sidebarOpen: true }" class="flex w-full">
            <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-30 w-64 bg-gradient-to-b from-gray-900 to-gray-800 text-gray-300 transition-transform duration-300 lg:translate-x-0 lg:static lg:inset-auto flex flex-col sidebar-scroll">
                ...
                <nav class="flex-1 overflow-y-auto overflow-x-hidden px-3 py-5 space-y-0.5 sidebar-scroll">
                    ...
                    <a href="{{ route('admin.pengaduan.index') }}" class="flex items-center px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 {{ request()->routeIs('admin.pengaduan*') ? 'bg-gray-700/80 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700/50' }}">
                        <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        <span>Kelola Pengaduan</span>
                    </a>
                    <a href="{{ route('admin.informasi.index') }}" class="flex items-center px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 {{ request()->routeIs('admin.informasi*') ? 'bg-gray-700/80 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700/50' }}">
