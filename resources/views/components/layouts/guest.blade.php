        ...
        <nav class="bg-white shadow-sm border-b">
                    ...
                    <div class="hidden sm:flex items-center space-x-6">
                        ...
                        <div class="relative" x-data="{ open: false }"
                            ...
                            <button type="button" @click="open = !open"
                                class="text-sm font-medium flex items-center gap-1 {{ request()->is('pengaduan') || request()->is('cek-pengaduan') ? 'text-blue-600' : 'text-gray-600 hover:text-gray-800' }}">
                                Layanan
                                <svg x-bind:class="open ? 'rotate-180' : ''" class="w-4 h-4 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                            </button>
                            <div x-show="open" x-cloak x-transition
                                class="absolute left-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-100 py-1 z-50">
                                <a href="{{ url('/pengaduan') }}" class="block px-4 py-2 text-sm {{ request()->is('pengaduan') ? 'text-blue-600 bg-blue-50' : 'text-gray-700 hover:bg-gray-50' }}">Pengaduan</a>
                                <a href="{{ url('/cek-pengaduan') }}" class="block px-4 py-2 text-sm {{ request()->is('cek-pengaduan') ? 'text-blue-600 bg-blue-50' : 'text-gray-700 hover:bg-gray-50' }}">Cek Pengaduan</a>
                            </div>
                        </div>
                    </div>
            ...
            <div id="mobile-menu" class="hidden sm:hidden border-t px-4 py-3 space-y-2">
                ...
                <div x-data="{ open: false }">
                    <button type="button" @click="open = !open"
                        class="block w-full text-left text-sm font-medium flex items-center justify-between {{ request()->is('pengaduan') || request()->is('cek-pengaduan') ? 'text-blue-600' : 'text-gray-600' }}">
                        Layanan
                        <svg x-bind:class="open ? 'rotate-180' : ''" class="w-4 h-4 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                    </button>
                    <div x-show="open" x-cloak class="pl-4 space-y-2 mt-1">
                        <a href="{{ url('/pengaduan') }}" class="block text-sm font-medium {{ request()->is('pengaduan') ? 'text-blue-600' : 'text-gray-600' }}">Pengaduan</a>
                        <a href="{{ url('/cek-pengaduan') }}" class="block text-sm font-medium {{ request()->is('cek-pengaduan') ? 'text-blue-600' : 'text-gray-600' }}">Cek Pengaduan</a>
                    </div>
                </div>
            </div>
        </nav>
