<x-layouts.admin>
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800 mt-2">Tambah Layer</h1>
    </div>

    <div class="bg-white rounded-lg shadow p-6 max-w-lg">
        <form action="{{ route('admin.layers.store') }}" method="POST">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <select name="geom_type" class="w-full border rounded-lg px-3 py-2" required>
                    @foreach ($geomTypes as $type)
                        <option value="{{ $type }}" {{ old('geom_type') == $type ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <div class="flex items-center gap-2">
                    <input type="color" name="warna" value="{{ old('warna', '#3388ff') }}" class="h-10 w-20 border rounded cursor-pointer">
                    <input type="text" name="warna_hex" value="{{ old('warna', '#3388ff') }}" class="border rounded-lg px-3 py-2 flex-1" maxlength="9">
                </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="opacity" value="{{ old('opacity', '1.00') }}" step="0.05" min="0" max="1" class="w-full border rounded-lg px-3 py-2" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Urutan</label>
                    <input type="number" name="order" value="{{ old('order', '0') }}" min="0" class="w-full border rounded-lg px-3 py-2" required>
                </div>
            </div>
            <div class="mb-4">
